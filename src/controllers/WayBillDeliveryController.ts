...
export const createMyWayBillDelivery = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    if (!sender?.address || !receiver?.address) {
      return res
        .status(400)
        .json({ message: "WayBill Sender & receiver address required" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Invalid WayBill address coordinates" });
    }
  ...
  const ref = await db.collection("wayBillDelivery").add(delivery);
...
return res.status(201).json({
  ...
  lastUpdated: data?.lastUpdated?.toDate?.() ?? null,
    ...
    return res.status(500).json({ message: "Failed to create waybill delivery" });
  }
};


/* ================= PAYSTACK CHECKOUT ================= */
export const createWayBillDeliveryCheckoutSession = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    const deliverySnap = await db.collection("wayBillDelivery").doc(deliveryId).get();

    if (!deliverySnap.exists) {
      return res.status(404).json({ message: "Waybill delivery not found" });
    }
    ...
    const response = await axios.post(
      "https://api.paystack.co/transaction/initialize",
      {
        ...
        callback_url: `godiyaoni://wayBillDelivery/${deliveryId}`,
...
/* ================= GET DELIVERY BY ID ================= */
export const getWayBillDeliveryById = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    const doc = await db.collection("wayBillDelivery").doc(deliveryId).get();
    ...
    return res.status(500).json({ message: "Failed to fetch WayBill delivery" });
  }
};

/* ================= PAYSTACK WEBHOOK ================= */
export const paystackWayBillDeliveryWebhookHandler = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    if (event.event === "charge.success") {
      ...
      if (deliveryId) {
        await db.collection("wayBillDelivery").doc(deliveryId).update({
          status: "paid",
