...
export const createMyMotorParkDelivery = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    if (!sender?.address || !receiver?.address) {
      return res
        .status(400)
        .json({ message: "MotorPark Sender & receiver address required" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Invalid MotorPark address coordinates" });
    }
  ...
  const ref = await db.collection("motorPatkDelivery").add(delivery);
...
/* ================= PAYSTACK CHECKOUT ================= */
export const createMotorParkDeliveryCheckoutSession = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    const deliverySnap = await db.collection("motorParkDelivery").doc(deliveryId).get();

    if (!deliverySnap.exists) {
      return res.status(404).json({ message: "MotorPark delivery not found" });
    }
    ...
    const response = await axios.post(
      "https://api.paystack.co/transaction/initialize",
      {
        ...
        callback_url: `godiyaoni://motorParkDelivery/${deliveryId}`,
...
/* ================= GET DELIVERY BY ID ================= */
export const geMotorParkDeliveryById = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    const doc = await db.collection("motorParkDelivery").doc(deliveryId).get();
    ...
    return res.status(500).json({ message: "Failed to fetch MotorPark delivery" });
  }
};

/* ================= PAYSTACK WEBHOOK ================= */
export const paystackMotorParkDeliveryWebhookHandler = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    if (event.event === "charge.success") {
      ...
      if (deliveryId) {
        await db.collection("motorParkDelivery").doc(deliveryId).update({
          status: "paid",
